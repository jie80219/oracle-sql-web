...
- **單一練習（37 題）** — 一個語法一次講清楚，每個主題 2～3 題，由淺入深。
- **組合練習（6 題）** — 一題要同時用上 LEFT JOIN + GROUP BY + NVL + CASE WHEN 這種，比較接近實務上真的會寫出來的查詢。
...
| 分類 | 內容 |
| --- | --- |
| JOIN | INNER / LEFT / RIGHT / FULL OUTER / 三表以上的多表 JOIN |
| 集合運算 | UNION、UNION ALL、MINUS、INTERSECT |
| 篩選 | IN / NOT IN（含 NULL 陷阱）、EXISTS / NOT EXISTS、OR 與 AND 的優先權 |
| 彙總 | COUNT / SUM / AVG / MAX / MIN、GROUP BY、HAVING |
| 其他 | DISTINCT、CASE WHEN、NVL / NVL2 |
| Oracle 慣用寫法 | JOIN UPDATE（可更新連接視圖）、MERGE / UPSERT |
...
- **DML 題**（JOIN UPDATE、MERGE）比對執行後受影響資料表的完整內容快照。
...
跑測試（驗證全部 43 題的參考答案都能執行、結果非空、且通過自己的批改）：
